...
export default function SettingsPage() {
  ...
  const notifications = [
    { key: "booking_submitted", label: "New booking submitted", email: true, sms: false },
    { key: "booking_approved", label: "Booking approved notification to renter", email: true, sms: true },
    { key: "booking_rejected", label: "Booking rejected notification to renter", email: true, sms: false },
    { key: "return_reminder", label: "Return reminder (3 days before due)", email: true, sms: true },
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-semibold text-primary">System Settings</h1>
        <p className="text-sm text-secondary mt-0.5">Platform configuration and administration</p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-gray-100 bg-gray-50">
              <tr>
                <th className="text-left px-4 py-3 text-xs text-secondary font-medium">Name</th>
                <th className="text-left px-4 py-3 text-xs text-secondary font-medium">Email</th>
                <th className="text-left px-4 py-3 text-xs text-secondary font-medium">Role</th>
                <th className="text-left px-4 py-3 text-xs text-secondary font-medium">Created</th>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-gray-50">
                  ...
                  <td className="px-4 py-3 text-secondary text-xs">{formatDate(u.createdAt)}</td>
      ...
      {activeTab === "Notifications" && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-gray-100 bg-gray-50">
              <tr>
                <th className="text-left px-4 py-3 text-xs text-secondary font-medium">Event</th>
                <th className="text-center px-4 py-3 text-xs text-secondary font-medium">Email</th>
                <th className="text-center px-4 py-3 text-xs text-secondary font-medium">SMS</th>
              </tr>
            </thead>
            <tbody>
              {notifications.map((n) => (
            ...
            {locations.map((l) => (
              <div key={l.id} className="flex items-center gap-3 px-5 py-3 border-b border-gray-50 last:border-0">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-secondary">{l.address}</p>
                </div>
      ...
      {activeTab === "Contract Templates" && (
        <div className="flex flex-col gap-4">
          {["Rental Agreement Template", "Rent-to-Sell Agreement Template"].map((tmpl) => (
            <div key={tmpl} className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
      ...
      <Modal open={locationModal.open} onClose={() => setLocationModal({ open: false })} title={locationModal.editing ? "Edit Location" : "Add Location"}>
        <div className="flex flex-col gap-4">
          <div>
            <label className="text-xs font-medium text-secondary block mb-1">Location Name</label>
